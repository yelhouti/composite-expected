...
class WithUUIDResourceIT {
    ...
    @Autowired
    private WithUUIDQueryService withUUIDQueryService;
    ...
    void createWithUUID() throws Exception {
        ...
        restWithUUIDMockMvc
            .perform(
                post("/api/with-uuids").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(withUUIDDTO))
            )
    ...
    void getAllWithUUIDS() throws Exception {
        ...
        restWithUUIDMockMvc
            .perform(get("/api/with-uuids?sort=uuid,desc"))
    ...
    void getWithUUID() throws Exception {
        ...
        // Get the withUUID
        restWithUUIDMockMvc
            .perform(get("/api/with-uuids/{id}", withUUID.getUuid()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    private void defaultWithUUIDShouldBeFound(String filter) throws Exception {
        restWithUUIDMockMvc
            .perform(get("/api/with-uuids?sort=uuid,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].uuid").value(hasItem(withUUID.getUuid().toString())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
        ...
        restWithUUIDMockMvc
            .perform(get("/api/with-uuids/count?sort=uuid,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    void getNonExistingWithUUID() throws Exception {
        // Get the withUUID
        restWithUUIDMockMvc.perform(get("/api/with-uuids/{id}", UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void updateWithUUID() throws Exception {
        // Initialize the database
        withUUIDRepository.saveAndFlush(withUUID);
        ...
        restWithUUIDMockMvc
            .perform(put("/api/with-uuids").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(withUUIDDTO)))
            .andExpect(status().isOk());
    ...
    @Test
    @Transactional
    void updateNonExistingWithUUID() throws Exception {
        int databaseSizeBeforeUpdate = withUUIDRepository.findAll().size();
    ...
    void deleteWithUUID() throws Exception {
        ...
        // Delete the withUUID
        restWithUUIDMockMvc
            .perform(delete("/api/with-uuids/{id}", withUUID.getUuid().toString()).accept(MediaType.APPLICATION_JSON))
