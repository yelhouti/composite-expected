...
import com.mycompany.myapp.service.criteria.TaskCommentCriteria;
...
class TaskCommentResourceIT {
    ...
    void createTaskComment() throws Exception {
        ...
        restTaskCommentMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(taskCommentDTO))
            )
    ...
    @Test
    @Transactional
    void putNewTaskComment() throws Exception {
        // Initialize the database
        taskCommentRepository.saveAndFlush(taskComment);
    ...
    void putWithMissingIdPathParamTaskComment() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTaskCommentMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(taskCommentDTO)))
