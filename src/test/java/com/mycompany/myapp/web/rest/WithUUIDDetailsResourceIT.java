...
class WithUUIDDetailsResourceIT {
    ...
    @Autowired
    private WithUUIDDetailsQueryService withUUIDDetailsQueryService;
    ...
    void getAllWithUUIDDetails() throws Exception {
        ...
        restWithUUIDDetailsMockMvc
            .perform(get("/api/with-uuid-details?sort=uuid,desc"))
            .andExpect(status().isOk())
    ...
    void getWithUUIDDetails() throws Exception {
        ...
        restWithUUIDDetailsMockMvc
            .perform(get("/api/with-uuid-details/{id}", withUUIDDetails.getUuid()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    private void defaultWithUUIDDetailsShouldBeFound(String filter) throws Exception {
        restWithUUIDDetailsMockMvc
            .perform(get("/api/with-uuid-details?sort=uuid,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].uuid").value(hasItem(withUUIDDetails.getUuid().toString())))
            .andExpect(jsonPath("$.[*].details").value(hasItem(DEFAULT_DETAILS)));
        ...
        restWithUUIDDetailsMockMvc
            .perform(get("/api/with-uuid-details/count?sort=uuid,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    void getNonExistingWithUUIDDetails() throws Exception {
        // Get the withUUIDDetails
        restWithUUIDDetailsMockMvc
            .perform(get("/api/with-uuid-details/{id}", UUID.randomUUID().toString()))
            .andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void updateWithUUIDDetails() throws Exception {
        // Initialize the database
        withUUIDDetailsRepository.saveAndFlush(withUUIDDetails);
    ...
    @Test
    @Transactional
    void updateNonExistingWithUUIDDetails() throws Exception {
        int databaseSizeBeforeUpdate = withUUIDDetailsRepository.findAll().size();
    ...
    void deleteWithUUIDDetails() throws Exception {
        ...
        restWithUUIDDetailsMockMvc
            .perform(delete("/api/with-uuid-details/{id}", withUUIDDetails.getUuid().toString()).accept(MediaType.APPLICATION_JSON))
