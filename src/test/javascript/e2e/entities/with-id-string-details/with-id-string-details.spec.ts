...
describe('WithIdStringDetails e2e test', () => {
  ...
  /* it('should create and save WithIdStringDetails', async () => {
        ...
        await promise.all([
            withIdStringDetailsUpdatePage.setDetailsInput('details'),
            withIdStringDetailsUpdatePage.withIdStringSelectLastOption(),
        ]);

        expect(await withIdStringDetailsUpdatePage.getDetailsInput()).to.eq('details', 'Expected Details value to be equals to details');
