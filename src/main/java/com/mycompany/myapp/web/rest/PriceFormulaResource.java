...
public class PriceFormulaResource {
    ...
    public ResponseEntity<PriceFormulaDTO> createPriceFormula(@Valid @RequestBody PriceFormulaDTO priceFormulaDTO)
        ...
        return ResponseEntity
            .created(new URI("/api/price-formulas/" + result.getMax()))
    ...
    public ResponseEntity<PriceFormulaDTO> updatePriceFormula(@Valid @RequestBody PriceFormulaDTO priceFormulaDTO)
        throws URISyntaxException {
        log.debug("REST request to update PriceFormula : {}", priceFormulaDTO);
        if (priceFormulaDTO.getMax() == null || !priceFormulaService.findOne(priceFormulaDTO.getMax()).isPresent()) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        PriceFormulaDTO result = priceFormulaService.save(priceFormulaDTO);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, priceFormulaDTO.getMax().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /price-formulas} : Updates given fields of an existing priceFormula.
     *
     * @param priceFormulaDTO the priceFormulaDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated priceFormulaDTO,
     * or with status {@code 400 (Bad Request)} if the priceFormulaDTO is not valid,
     * or with status {@code 404 (Not Found)} if the priceFormulaDTO is not found,
     * or with status {@code 500 (Internal Server Error)} if the priceFormulaDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/price-formulas", consumes = "application/merge-patch+json")
    public ResponseEntity<PriceFormulaDTO> partialUpdatePriceFormula(@NotNull @RequestBody PriceFormulaDTO priceFormulaDTO)
        throws URISyntaxException {
        log.debug("REST request to update PriceFormula partially : {}", priceFormulaDTO);
        if (priceFormulaDTO.getMax() == null || !priceFormulaService.findOne(priceFormulaDTO.getMax()).isPresent()) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }

        Optional<PriceFormulaDTO> result = Optional.ofNullable(priceFormulaService.partialUpdate(priceFormulaDTO));

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, priceFormulaDTO.getMax().toString())
        );
    }
